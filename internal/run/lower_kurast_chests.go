...
func (run LowerKurastChests) Run(parameters *RunParameters) error {
	...
	for _, bonfirePos := range bonFirePositions {
		...
		for len(objects) > 0 {
			...
			if err != nil {
				run.ctx.Logger.Warn("Failed interacting with object", "supervisor", run.ctx.Name, "object", closestObject.Name, "area", run.ctx.Data.PlayerUnit.Area.Area().Name, "error", err)
			}
